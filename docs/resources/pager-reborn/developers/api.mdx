...
:::info
The API requires Pager Reborn, and does not work for Sonoran Edition. For more info, [see here](../index.md#available-versions).
:::
...
#### Return Value
- `200 - OK`
	- Returns a JSON object where each key is a capcode/address and each value is a list of player server IDs currently assigned to that capcode.
- `401 - Unauthorized`
	- API access token does not match, or caller IP is not allowed by [`ic_pr_whitelistedIps`](../config.md#api-whitelist).
- `404 - Not Found`
	- Endpoint not found, or API access token is not configured.
	- May return a message in the response body.
